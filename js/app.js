...
/* ====== EKRAN DURUMU (fix) ====== */
let currentScreen = "screenName"; // default; render() bunu isim durumuna göre ayarlar
...
/* --- yeni dağılım: erken dönemde de azıcık zor gelsin --- */
function pickWeighted(poolEasy){
  const full = [2,3,4,5,6,7,8,9,10];
  const useEasy = Math.random() < 0.7; // %70 kolay havuz, %30 full
  const pool = useEasy ? poolEasy : full;
...
function pickQuestion(){
  ...
  // marathon full tablo, diğerlerinde weighted
  const basePool = (mode === "marathon") ? [2,3,4,5,6,7,8,9,10] : ph.pool;

  const a = (mode === "marathon") ? basePool[Math.floor(Math.random()*basePool.length)] : pickWeighted(basePool);
  const b = (mode === "marathon") ? basePool[Math.floor(Math.random()*basePool.length)] : pickWeighted(basePool);
...
function onAnswer(val, btnEl){
  ...
  if(correct){
    ...
    if(mode === "master"){
      ...
      mtoast(`Maraton! +${pts.leaves}🍃 +${pts.xp}✨`);
    } else {
    ...
    setTimeout(() => {
      ...
      // FIX: render artık ekrana dokunmuyor; biz ekranı burada sabitliyoruz
      render();
      show("screenMath");
      renderMath();
    }, 520);
...
function render(){
  ...
  // FIX: render artık "hangi ekranda olduğuna" karar vermiyor
  // Sadece sayı/görsel günceller. Ekranı show(...) yönetir.
}
...
// First paint: decide screen
(function initScreen(){
  render();
  if(state.koala && state.koala.name){
    show("screenHome");
  } else {
    show("screenName");
  }
